...
class MCTS(object):
    def __init__(self, policy_network=None, value_network=None, seconds_per_move=5, rollout_policy=True, use_depth=False, random_network=None):
        ...
        if rollout_policy:
            self.play_valid_move = self.use_policy
        else:
            self.play_valid_move = self.use_random
        if self.use_depth:
            self.depth = 1
        else:
            self.depth = 15
    ...
    def suggest_move(self, game):
        ...
        if self.root is None:
            move_probs = self.random_network.evaluate(game)
    ...
    def shift_root(self, move):
        ...
        if move in self.current_root.children:
            ...
            print ("\n\n\n\nFatal Error\n\n\n\n")
            # move_probs = self.policy_network.evaluate(self.current_root.game)
    ...
    def use_policy(self, game):
        move_probs = self.policy_network.evaluate(game)
        game.make_valid_move(sorted_moves(move_probs)[0])
        return game

    def use_random(self, game):
        game.make_valid_move(random.choice(Othello.legal_moves(game.player, game.board)))
        return game
