...
def create_model():
    model = Sequential()
    model.add(Convolution2D(64, 4, 4, activation='sigmoid', border_mode='valid', input_shape=(11, 10, 10)))
    # model.add(MaxPooling2D(pool_size=(2, 2)))

    model.add(Convolution2D(256, 3, 3, activation='sigmoid', border_mode='valid'))
...
if __name__=='__main__':
    ...
    f2 = file("cat_test_set_10.npy", "rb")
    data_test = np.load(f2)
    label_test = np.load(f2)
    X_train = np.asarray(data_train[0:660000,:,:,:])
    X_test = np.asarray(data_test[0:90000,:,:,:])
    Y_train = np_utils.to_categorical(label_train[0:660000], 17)
    Y_test = np_utils.to_categorical(label_test[0:90000], 17)
    ...
    model.save(time.strftime('./model/CNN_cat_10_model_L_%Y%m%d%H%M.h5'))
    ...
    plt.ylim([0.1, 1.0])
    plt.tight_layout()
    plt.savefig(time.strftime('./figures/CNN_cat_10_tuning_epoch_learning_curve_%Y%m%d%H%M.png'), dpi=300)
    plt.show()
