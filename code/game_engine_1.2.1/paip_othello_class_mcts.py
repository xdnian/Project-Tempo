...
class othello():
    ...
    def play_with_MCTS(self, black_strategy, white_strategy):
        self.restart()
        strategy = lambda who: black_strategy if who == BLACK else white_strategy

        while self.__player is not None:
            self_strategy = strategy(self.__player)
            oppo_strategy = strategy(othello.opponent(self.__player))

            if isinstance(self_strategy, MCTS):
                move = self_strategy.suggest_move(self)
                self_strategy.shift_root(move)
            else:
                move = othello.get_move(self_strategy, self.__player, self.__board)
...
class MCTS(object):
    ...
    def suggest_move(self, paip):
        ...
        if self.root is None:
            ...
            self.root = MCTSNode.root_node(paip, move_probs)
        ...
        #     print i, self.current_root.children[i].Q, self.current_root.children[i].action_score, self.current_root.children[i].N
    ...
    def shift_root(self, move):
        ...
        if move in self.current_root.children:
            ...
            print "\n\n\n\nFatal Error\n\n\n\n"
            # move_probs = self.policy_network.evaluate(self.current_root.paip)
